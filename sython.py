...
async def join_channel():
    try:
        await sython(JoinChannelRequest("@xx_YaBh"))
...
async def _(event):
    ...
    await event.send_file("me","https://t.me/xx3bD/203",caption=f"""
**༗ 𝘵𝘩𝘦 𝘴𝘰𝘶𝘳𝘤𝘦 𝘠𝘢𝘉𝘩 𝘸𝘢𝘴 𝘳𝘶𝘯𝘯𝘪𝘯𝘨
༗ 𝘱𝘪𝘯𝘨 : `{ms}`
༗ 𝘥𝘢𝘵𝘦 : `{m9zpi}`
༗ 𝘪𝘥 : `{event.sender_id}`
...
async def OwnerStart(event):
    ...
    if sender.id == ownerhson_id :
        order = await event.reply('- ها ضلعي يمك عَبود يابه؟ | @xx_YaBh')
...
async def _(event):
        ...
        await sython.send_message('@EEObot', 'جاري التجميع بواسطة | 𝑺𝒐𝒖𝒓𝒄𝒆 𝒀𝒂𝑩𝒉')
        channel_entity = await sython.get_entity(bot_username)
...
async def _(event):
        ...
        await sython.send_message('@A_MAN9300BOT', 'جاري التجميع بواسطة | 𝑺𝒐𝒖𝒓𝒄𝒆 𝒀𝒂𝑩𝒉')
        channel_entity = await sython.get_entity(bot_usernamee)
...
async def join_channel():
    try:
        await sython(JoinChannelRequest("@mmaahg"))
    except BaseException:
        pass
...
async def _(event):
      await event.reply("""
༗ 𝑾𝒆𝒍𝒄𝒐𝒎𝒆 𝒕𝒐 𝑺𝒐𝒖𝒓𝒄𝒆 𝒀𝒂𝑩𝒉?
༗ 𝑻𝒉𝒆 𝒃𝒆𝒔𝒕 𝑺𝒐𝒖𝒓𝒄𝒆 𝒊𝒏 𝑻𝒆𝒍𝒆𝒈𝒓𝒂𝒎
༗ 𝑻𝒉𝒆 𝒅𝒆𝒗𝒆𝒍𝒐𝒑𝒆𝒓 𝒐𝒇 𝒕𝒉𝒆 𝑺𝒐𝒖𝒓𝒄𝒆 : @xx_YaBh 
"""
)

@sython.on(events.NewMessage(outgoing=True, pattern=".مطور"))
async def _(event):
      await event.reply("""ᴛʜᴇ ᴅᴇᴠ ꜱᴏᴜʀᴄᴇ : @xx_YaBh"""
)
